...
export const projects: Project[] = [
  {
    ...
    details: [
      ...
      },
      {
        ...
        problem:
          '클라이언트에서 수행되는 오디오 변환과 미디어 처리 로직은 브라우저 API와 비동기 흐름에 의존해 동작을 안정적으로 검증할 필요가 있었습니다.',
        solution:
          '`vitest`와 `@testing-library/react`를 사용해 오디오 변환 서비스와 녹음 훅의 핵심 로직을 단위 테스트로 검증했습니다. FFmpeg 인스턴스와 브라우저 API를 Mock으로 대체해 WebM → MP3 변환 흐름과 녹음 상태 전이, 오류 처리 로직을 테스트하고, 이미지 포맷 변환 및 압축 유틸리티의 동작도 함께 검증했습니다.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    details: [
      {
        title: '모노레포 기반 풀스택 프로젝트 구조 구축',
        problem:
          '웹, 모바일 앱, API 서버, AI 서버가 함께 동작하는 구조에서 프로젝트 간 설정과 의존성을 일관되게 관리할 필요가 있었습니다.',
        solution:
          '각 프로젝트를 모노레포로 통합했습니다. TypeScript 설정과 린트 규칙, 공통 의존성을 중앙에서 관리해 프로젝트 간 환경을 일관되게 유지했습니다.',
      },
      {
        title: '백그라운드 프레임 분석 기반 얼굴 촬영 가이드 UI 구현',
        problem:
          '얼굴 인식 정확도를 위해 촬영 시 얼굴 크기, 기울기, 여백 조건 등을 확인할 필요가 있었습니다.',
        solution:
          '`react-native-vision-camera`의 Frame Processor를 활용해 카메라 프레임을 Worklet 스레드에서 처리하도록 구현했습니다. 얼굴 위치와 크기, 기울기 조건을 실시간으로 분석하고 가이드 메시지를 UI에 반영하여 사용자가 적절한 위치에서 얼굴을 촬영할 수 있도록 했습니다.',
      },
      {
        title: 'EXIF 회전 메타데이터로 인한 얼굴 인식 오류 해결',
        problem:
          '모바일에서 촬영한 이미지로 얼굴을 등록할 때 일부 환경에서 인식 정확도가 낮아지는 문제가 있었습니다.',
        solution:
          '디버깅 중 모바일 이미지에 포함된 EXIF 회전 메타데이터가 FastAPI 서버에서 반영되지 않아 이미지 방향이 뒤집힌 상태로 처리되고 있음을 확인했습니다. FastAPI 서버에서 이미지를 정방향으로 보정하여 인식률 문제를 해결했습니다.',
      },
    ],
  },
  {
    ...
    details: [
      ...
      {
        title: '디바운스 기반 검색 API 호출 최적화',
        problem:
          '검색어 자동 완성 기능에서 사용자의 입력마다 API 요청을 보내면 불필요한 네트워크 요청이 발생할 수 있었습니다.',
        solution:
          '입력 디바운스를 적용해 일정 시간 입력이 멈춘 경우에만 API를 호출하도록 했고, 쿼리 캐싱을 적용해 동일한 검색어 요청에 대해 네트워크 호출을 줄이고 응답 속도를 개선했습니다.',
      },
    ],
  },
  {
    ...
    details: [
      ...
      {
        title: '다양한 화면 크기를 고려한 반응형 UI 유틸리티 구현',
        problem:
          '기기 해상도와 화면 비율이 달라지면서 동일한 UI가 기기마다 다른 크기로 보이는 문제가 있었습니다.',
        solution:
          '디자인 기준 해상도를 기반으로 화면 비율을 계산하는 유틸리티를 구현해 UI 요소 크기를 동적으로 변환하도록 했습니다. 또한 텍스트 길이에 따라 자동 스크롤을 처리하는 컴포넌트와 단어 단위 줄바꿈 컴포넌트를 구현해 다양한 화면 크기에서도 텍스트 레이아웃이 안정적으로 유지되도록 했습니다.',
      },
    ],
  },
];
